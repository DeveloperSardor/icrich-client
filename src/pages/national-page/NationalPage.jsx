...
const NationalPage = () => {
  ...
  const handleNextData = () => {
    if (currentIndex < unescoData.length - 1) {
  ...
  useEffect(() => {
    const fetchUnescoData = async () => {
      try {
        ...
        setUnescoData(response.data.data);
      } catch (error) {
  ...
  if (!unescoData || unescoData.length === 0) {
    return (
      <div className="national-container">
  ...
  const images = unescoData[currentIndex]?.images || [];
  ...
  const getText = (field) => {
    const item = unescoData[currentIndex];
    if (!item) return '';
    ...
    <div className="national-container">
      ...
      <div className="video-section">
        <iframe
          width="853"
          height="480"
          src={getYouTubeEmbedUrl(unescoData[currentIndex]?.youtube_link)}
          title="National list video"
          frameBorder="0"
      ...
      <div className="navigation-buttons">
        ...
        <span className="page-indicator">
          {currentIndex + 1} / {unescoData.length}
        </span>
        <button onClick={handleNextData} disabled={currentIndex === unescoData.length - 1}>
